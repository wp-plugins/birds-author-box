...
if ( ! class_exists( 'Birds_Settings_Bab' ) ) {

    class Birds_Settings_Bab {
        ...
        $empty = true,
        $notices = array();
        const text_domain = 'birds-authorbox';
...
<?php }
        ...
        public function admin_init()
        {
            foreach ( $this->settings as $setting => $section ) {
                ...
                if ( ! empty( $section['fields'] ) ) {
                    $this->empty = false;
                    $values = get_setting_bab( $setting );
                    foreach ( $section['fields'] as $name => $field ) {
                        $id = $setting . '_' . $name;
                        $field = array_merge( array(
                            'id'    => $id,
    ...
    add_action( 'activated_plugin', array( 'Birds_Settings_Bab', 'plugin_priority' ) );

    function get_setting_bab( $setting, $option = false )
    {
        $setting = get_option( $setting );
        if ( is_array( $setting ) ) {
            if ( $option ) {
                return isset( $setting[$option] ) ? Birds_Settings_Bab::parse_multi( $setting[$option] ) : false;
            }
            foreach ( $setting as $k => $v ) {
                $setting[$k] = Birds_Settings_Bab::parse_multi( $v );
            }
            return $setting;
        }
    ...
    function create_settings_page_bab( $page = 'custom_settings', $title = null, $menu = array(), $settings = array(), $args = array() )
    {
        return new Birds_Settings_Bab( $page, $title, $menu, $settings, $args );
    }
