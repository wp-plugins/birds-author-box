...
function birds_get_related_author_posts()
{
    ...
    global $wpdb;
    $latest = get_option('bab_latest_number');
    $yesnolast = get_option('bab_latest_radio_buttons');
    ...
    $authors_posts = get_posts(array(
        ...
        'posts_per_page' => $latest,
        'orderby' => 'post_date',
        'order' => 'DESC'
    ));
    if ($yesnolast == 'yes')
    {
        $output = '<p class="alinea">';
...
function birds_authorbox_add_post_content($content)
{
    ...
    global $wpdb;
    $posts_checkbox = get_option('bab_posts_checkbox');
    $pages_checkbox = get_option('bab_pages_checkbox');
    $avatarcss = get_option('bab_avatar_radio_buttons');
    $yesnowebsite = get_option('bab_website_radio_buttons');
    $yesnolast = get_option('bab_latest_radio_buttons');
    $iconsset = get_option('bab_icons_select_box');
    if ($iconsset == 'circle')
    {
    ...
    { // If the page is not a WooCommerce one, let's do the trick!
        if ((is_single() && ($posts_checkbox != '')) || (is_page() && ($pages_checkbox != '') && !is_home() && !is_front_page()))
        {
            ...
            {
                $content.= '<p class="alinea"><a class="box_links" href="' . get_the_author_meta('user_url') . '" target="_blank">' . __("Visit the author's website", "birds-authorbox") . '</a><br /><br />';
            }
            else
            {
                $content.= '<p class="alinea"><br /> ';
            }
